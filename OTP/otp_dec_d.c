...
** Name: Steven Cruz
** Date: 11/30/2018
** Description: file for the decode portion
...
void decryptText(char text[], char key[]){
    ...
    for (i = 0; i < length; i++){
        ...
        decryptedTextInt = (textInt - keyInt) % 27;
        if (decryptedTextInt < 0){ decryptedTextInt += 27; }
        text[i] = intToChar(decryptedTextInt);
    }
    text[i] = '\0';
    return;
}
